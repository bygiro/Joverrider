...
/**
 * Joverrider Component JoverriderModelItem Model
 *
 * @package		Joomla
 * @subpackage	Joverrider
 *
 */
class JoverriderModelItem extends JModelItem
{
	/**
	 * id
	 *
...
//		$itemParams->loadString((isset($item->attribs)?$item->attribs:$item->params));
	...
	function _deleteFiles($cids, $fileFields)
	{
		...
		{
			$dir = JPath::clean(JPATH_SITE . DS . $config->get('upload_dir_' . $this->_name_plur . '_' . $fieldName, '[COM_SITE]' .DS. 'files' .DS. $this->_name_plur . '_' . $fieldName));
			...
			foreach($files as $fileObj)
			{
				...
				if (!JoverriderFile::deleteFile($imagePath, $op))
					$errors = true;
	...
	{
		$globalParams = JComponentHelper::getParams('com_joverrider', true);
		$this->setState('params', $globalParams);



		if (defined('JDEBUG'))
			$_SESSION["Joverrider"]["Model"][$this->getName()]["State"] = $this->state;
